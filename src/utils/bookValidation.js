...
async function validateTypeidFromBooks(typeId) {
    if(typeof typeId !== 'number' || !Number.isInteger(typeId <= 0)) {
        return 'typeID doit être un id';
    }
//le verifie si le type existe en bdd
    const type = await TypeError.findByPk(typeId);
    if(!type){
